...
export function AddressArrayInput({ countName, fieldString, fieldName, data, startingInputs = new Array<AddressType>(1) }: AddressArrayInputProps): ReactNode {
    ...
    for (let i = 0; i < inputs.length; i++) {
        inputs[i]=(<div className="grid grid-cols-1 md:grid-cols-2 col-span-2 gap-4 border-black border-solid border-2" key={`${fieldName}-container-${i}`}>
            ...
            <input className="bg-slate-100" required defaultValue={startingInputs.length > i ? startingInputs[i].address : ""}
    ...
    return (<div className="grid grid-cols-2 gap-2 col-span-2">
        <button className="bg-green-500 rounded-md text-lg p-1 col-span-2 md:col-span-1" onClick={() => {setInputs(new Array<ReactNode>(inputs.length + 1)); inputIds.push("new"); setIds(new Array<string>(...inputIds));}} type="button">Добави Адрес</button>
        <button className="bg-green-500 rounded-md text-lg p-1 col-span-2 md:col-span-2" onClick={() => {setInputs(new Array<ReactNode>(Math.max(inputs.length - 1, 0))); inputIds.pop(); setIds(new Array<string>(...inputIds));}} type="button">Премахни Последен Адрес</button>
