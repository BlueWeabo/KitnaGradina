...
export function AddressArrayInput({ countName, fieldString, fieldName, data, startingInputs }: AddressArrayInputProps): ReactNode {
    const [inputs, setInputs] = useState<Array<ReactNode>>(new Array<ReactNode>(startingInputs.length));
    const [inputIds, setIds] = useState<Array<string>>(new Array<string>(...(startingInputs.map(op=>op.id === null ? "" : op.id.toString()))));
    for (let i = 0; i < inputs.length; i++) {
        inputs[i]=(<div className="grid grid-cols-2 col-span-2 gap-4 border-black border-solid border-2" key={`${fieldName}-container-${i}`}>
            ...
            <select key={`${fieldName}-${i}`} name={`${fieldName}_${i}`} required value={inputIds[i]} data-index={i} onChange={(e)=>{
                ...
                <option key={`${fieldName}-${i}-none`} value="">Нов Адрес</option>
                {data.map(product => <option key={`${fieldName}-${i}-${product.id}`} value={product.id?.toString()} >{product.address}</option>)}
            </select>
            <label key={`${fieldName}-quantl-${i}`}>Адрес</label>
            <input className="bg-slate-100" required
                key={`${fieldName}_amount_${i}`} type="text" step={0.01} name={`${fieldName}_${i}_address`} />
